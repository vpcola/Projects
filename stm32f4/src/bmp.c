...
int32_t Capture_Image_TO_Bmp(void)
{
  ...
  sprintf(file_str, "pic%d.bmp", (int) pic_counter);
  ...
  for (j = 0; j < 240; j++) {
    ...
    ret = f_write(&file, image_buf, 640, (UINT *)&bw);
  }
#endif
...
static int32_t get_pic_count(void)
{
  ...
  if (ret != FR_OK) {
    ...
    if (ret == FR_OK) {
      pic_counter = 0;
      ret = f_write(&file, &pic_counter, sizeof(uint32_t), (UINT *)&bw);
      f_close(&file);
      f_mount(0, NULL);
      return pic_counter;
    } else {
    ...
    ret = f_read(&file, &pic_counter, sizeof(uint32_t), (UINT *)&bw);
    f_close(&file);
    f_mount(0, NULL);
    return pic_counter;
  }
